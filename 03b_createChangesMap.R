...
see_changes <- function(spc){
 ...
 #spc <- spcs[1] # activate for testing
  cat('-------------------------------------------------------------\n')
  cat('To start ', spc, '\n')
  cat('-------------------------------------------------------------\n')
  ...
  #tbl <- mutate(tbl, ratio = ((y2091 - y2011) / y2011)* 100)  #change for 2091
  tbl <- mutate(tbl, ratio = (y2091 / y2011)* 100)  #change for 2091
  ...
  gbn <- ggplot() + 
    ...
    scale_fill_manual(values = c('#D73027','#f4f4f4', '#1A9870')) +
    ggtitle(label = spc) +
    theme_bw() +
